...
#AutoIt3Wrapper_Icon=..\..\torLogo.ico
...
Func MRC_log($path, $success, $key, $log)
	Local $tempPath = $path & "\ErrorLog.ini"
	Local $section = "Fail"
	If $success Then
		$section = "Success"
	EndIf

	IniWrite($tempPath, $section, $key, $log)
...
Func BTNTrigger_ConvertFolder($inputDir, $outputDir)
		...
		For $YDD In $aYDD
			...
			If $YDDCategory == -1 Then
				MRC_log($outputDir, False, $YDD, "Failed to get YDD Category")
				ContinueLoop
			EndIf

			Local $YDDIndex = GetYDDNumber($YDD)
			If $YDDIndex == -1 Then
				MRC_log($outputDir, False, $YDD, "Failed to get YDD Index")
				ContinueLoop
			EndIf
			...
			Else
				MRC_log($outputDir, False, $YDD, "Failed to get Gender")
				ContinueLoop
			EndIf
...
Func GetYDDNumber($file)
	...
	If UBound($aSearchStringRaw) < 2 Then
		return -1
	EndIf
